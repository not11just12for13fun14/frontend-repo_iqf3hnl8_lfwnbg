    ...
    <div className="fixed inset-x-0 top-0 z-30">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <motion.div
          ...
          className="mt-6 rounded-2xl border border-white/20 bg-white/40 backdrop-blur supports-[backdrop-filter]:bg-white/40 shadow-lg"
        >
          <div className="flex items-center justify-between px-6 py-4">
            <a href="#" className="flex items-center gap-2">
              <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 240, damping: 14 }} className="h-8 w-8 rounded-lg bg-gradient-to-tr from-indigo-600 to-blue-500" />
  ...
  return (
    <section className="relative h-[88vh] w-full overflow-hidden">
      ...
      <div className="relative z-10 mx-auto flex h-full max-w-7xl flex-col items-center justify-center px-6 text-center">
        <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white/70 px-3 py-1 text-xs font-medium text-slate-600 shadow">
          <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" /> Now supporting AWS, Azure, and GCP
        </motion.div>
        <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, type: 'spring', stiffness: 140 }} className="mt-6 text-4xl font-semibold leading-tight text-slate-900 sm:text-5xl md:text-6xl">
          MultiCloud & MultiAccount Management
        </motion.h1>
        <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mt-4 max-w-2xl text-slate-600 sm:text-lg">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="mt-8 flex flex-col items-center gap-3 sm:flex-row">
          ...
          <a href="/dashboard" className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-5 py-3 text-white shadow hover:bg-indigo-500">
            Preview App
          </a>
        </motion.div>
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }} className="mt-6 flex items-center gap-6 text-xs text-slate-500">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className="pointer-events-none absolute inset-0">
        <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }} className="absolute -left-10 top-20 h-40 w-40 rounded-full bg-gradient-to-tr from-indigo-500/20 to-blue-500/20 blur-2xl" />
        <motion.div animate={{ y: [0, 12, 0] }} transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut' }} className="absolute right-0 bottom-10 h-56 w-56 rounded-full bg-gradient-to-tr from-cyan-400/20 to-emerald-400/20 blur-2xl" />
      </motion.div>
    </section>
  )
}
    ...
    <section className="bg-white">
      <div className="mx-auto max-w-7xl px-6 py-10">
        ...
        <div className="mt-6 grid grid-cols-2 items-center gap-6 opacity-70 sm:grid-cols-3 md:grid-cols-6">
          {logos.map((l, i) => (
            <motion.div key={l} initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="flex h-10 items-center justify-center rounded-md border border-slate-100 bg-white">
    ...
    <section id="features" className="bg-gradient-to-b from-white to-slate-50">
      <div className="mx-auto max-w-7xl px-6 py-20">
        <div className="mx-auto max-w-2xl text-center">
          <motion.h2 initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-3xl font-semibold text-slate-900 sm:text-4xl">All your clouds. One control plane.</motion.h2>
          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="mt-3 text-slate-600">Designed for platform, FinOps, and security teams to move with confidence.</motion.p>
        </div>
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {items.map((f, i) => (
            <motion.div key={f.title} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="group rounded-2xl border border-slate-200 bg-white p-6 shadow-sm hover:shadow-md">
              <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-slate-50 group-hover:scale-105 transition-transform">{f.icon}</div>
        ...
        <div className="grid gap-10 md:grid-cols-2 md:items-center">
          <div>
            ...
            <div className="mt-6 grid grid-cols-3 gap-3">
              {providers.map((p, i) => (
                <motion.div key={p.name} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.04 }} className={`rounded-xl p-4 text-center text-sm font-semibold text-white shadow bg-gradient-to-br ${p.color}`}>
                  {p.name}
                </motion.div>
              ))}
            </div>
          </div>
          <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-2xl border border-slate-200 bg-slate-50 p-6 shadow-sm">
            <div className="aspect-[4/3] w-full rounded-xl bg-white p-6 shadow-inner">
              <div className="grid h-full w-full grid-cols-3 gap-3">
                {Array.from({ length: 9 }).map((_, i) => (
                  <motion.div key={i} initial={{ opacity: 0, y: 6 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.03 }} className="rounded-lg border border-slate-200 bg-slate-50" />
                ))}
              </div>
            </div>
...
function DashboardPreview() {
  return (
    <section id="dashboard" className="bg-gradient-to-b from-slate-50 to-white">
      <div className="mx-auto max-w-7xl px-6 py-20">
        <div className="mx-auto max-w-2xl text-center">
          <motion.h2 initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-3xl font-semibold text-slate-900 sm:text-4xl">A dashboard your execs will love</motion.h2>
          <p className="mt-3 text-slate-600">Real-time posture, spend, and usage by account and cloud with drill-downs and filters.</p>
        </div>
        <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-10 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl">
          <div className="flex flex-col gap-6 p-6 md:flex-row">
            <div className="md:w-2/3">
              <motion.div initial={{ scale: 0.98 }} whileInView={{ scale: 1 }} viewport={{ once: true }} transition={{ type: 'spring', stiffness: 160, damping: 18 }} className="h-64 rounded-xl bg-gradient-to-br from-indigo-500/10 to-blue-500/10 ring-1 ring-inset ring-slate-200" />
              <div className="mt-4 grid grid-cols-3 gap-3">
                {Array.from({ length: 6 }).map((_, i) => (
                  <motion.div key={i} initial={{ opacity: 0, y: 6 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.04 }} className="h-20 rounded-lg bg-slate-50 ring-1 ring-inset ring-slate-200" />
                ))}
              </div>
            </div>
            <div className="md:w-1/3">
              <div className="space-y-3">
                {[
                  ...
                  <motion.div key={k} initial={{ opacity: 0, x: 8 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 p-4">
        ...
        <div className="mt-6 text-center">
          <a href="/dashboard" className="inline-flex items-center gap-2 rounded-lg bg-slate-900 px-5 py-3 text-white shadow hover:bg-slate-800">Open Preview Dashboard</a>
        </div>
      </div>
    </section>
  )
...
function Pricing() {
  ...
  return (
    <section id="pricing" className="bg-white">
      <div className="mx-auto max-w-7xl px-6 py-20">
        <div className="mx-auto max-w-2xl text-center">
          <motion.h2 initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-3xl font-semibold text-slate-900 sm:text-4xl">Simple, transparent pricing</motion.h2>
          <p className="mt-3 text-slate-600">Start free. Scale when you need to.</p>
        </div>
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {tiers.map((t, i) => (
            <motion.div key={t.name} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm ring-1 ring-transparent hover:ring-indigo-200">
...
function CTA() {
  return (
    <section id="get-started" className="relative overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-6 py-20">
        <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-2xl bg-slate-900 p-8 sm:p-12">
